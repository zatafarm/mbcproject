...
public class FileUtils {
    ...
    private final String uploadPath = Paths.get("C:", "mbc", "upload-files").toString();
    ...
    public void deleteFiles(final List<FileResponse> files) {
        ...
        for (FileResponse file : files) {
        	String uploadedDate = file.getCreatedDate().split(" ")[0].replace("-", "").substring(2);
     ...
     * @return 첨부파일(리소스)
     */
    public Resource readFileAsResource(final FileResponse file) throws Exception {
    	String uploadedDate = file.getCreatedDate().split(" ")[0].replace("-", "").substring(2);
        String filename = file.getSaveName();
        Path filePath = Paths.get(uploadPath, uploadedDate, filename);
        try {
            Resource resource = new UrlResource(filePath.toUri());
            if (resource.exists() == false || resource.getFile() == null) {
