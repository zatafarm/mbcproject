...
<title>vacation Apply List</title>
<style>
.inline-container {
	display: flex;
	align-items: center;
...
<body class="dashboard dashboard_1">
											...
											<div class="margin_0">
												...
												<div class="inline-container">
													...
													<button type="button" class="btn btn-success"
														onclick="location.href='ptoApply'">휴가 신청</button>
													<button type="button"
														class="btn btn-danger ptocancle">신청 취소</button>
												</div>
											</div>
										</div>
																...
																<c:forEach var="aprv" items="${ptolist}">
																	<tr class="record"
																		...
																		<td onclick='event.cancelBubble=true;'><input
																			type="checkbox" name="chk" value="${aprv.pno}"></td>
																		<td>${aprv.sttsnm}</td>
																		<td>${aprv.ptotpnm}</td>
																		<td>${aprv.reqdt}</td>
																		<td>${aprv.apprvldt}</td>
	...
	$(document).ready(function() {
	    ...
	    $(document).on('click', '.ptocancle', function(event) {
	        ...
	        if (hasNonPending) {
	            alert('대기 상태가 아닌 항목은 삭제할 수 없습니다.');
	        } else {
